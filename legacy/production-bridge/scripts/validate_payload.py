    ...
    for index, signal in enumerate(signals):
        ...
        if severity is not None:
            if not isinstance(severity, int):
                errors.append(
                    f"signals[{index}].severity must be integer"
                )
        ...
        if confidence is not None:
            if not isinstance(confidence, (int, float)):
                errors.append(
                    f"signals[{index}].confidence must be numeric"
                )
        ...
        if (
            ...
            and direction not in VALID_DIRECTION
        ):
            errors.append(
...
def main():
    if len(sys.argv) != 2:
        print(
            "Usage: validate_payload.py payload.json"
        )
        sys.exit(2)
